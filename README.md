...
- **`Modelo.EmpleadoFijo` (Extiende de Modelo.Empleado)**:
    - Atributo específico: `salarioBase` (double).
    - Implementa `calcularSalario()` retornando el `salarioBase`.
- **`Modelo.EmpleadoPorHoras` (Extiende de Modelo.Empleado)**:
...
- **Calidad:** Agregar comentarios JavaDoc en la clase `Modelo.Empresa` y manejar excepciones básicas (por ejemplo, validar que no se ingresen montos negativos).