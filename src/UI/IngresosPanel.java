...
public class IngresosPanel extends JPanel {
    ...
    public IngresosPanel() { this(null); }

    public IngresosPanel(Empresa empresa) {
    ...
    private JPanel buildHeader() {
        ...
        JLabel title = new JLabel("↑  Ingresos");
        title.setFont(new Font("Segoe UI", Font.BOLD, 28));
        title.setForeground(TEXT_PRIMARY);
        JLabel subtitle = new JLabel("Gestión de ingresos y ventas");
        subtitle.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        subtitle.setForeground(TEXT_MUTED);
        titles.add(title, BorderLayout.NORTH);
        titles.add(subtitle, BorderLayout.SOUTH);

        JButton btnAdd = createButton("+ Agregar Ingreso", COLOR_GREEN);
    ...
    private JPanel buildMetricsRow() {
        List<Transaccion> ingresos = getIngresos();
        double total  = ingresos.stream().mapToDouble(Transaccion::getMonto).sum();
        int    count  = ingresos.size();
        double avg    = count > 0 ? total / count : 0;
        String topCat = ingresos.stream()
        ...
        row.add(metricCard("↑", "Total ingresos",      fmt(total),            COLOR_GREEN));
        row.add(metricCard("≡", "Nº de registros",     String.valueOf(count), ACCENT));
        row.add(metricCard("≈", "Promedio por ingreso", fmt(avg),             TEXT_MUTED));
        row.add(metricCard("★", "Categoría líder",     topCat,                ACCENT));
        return row;
    }
    ...
    private JPanel buildFooter() {
        ...
        JLabel stats = new JLabel("Total registros: " + ingresos.size()
    ...
    public void mostrarFormularioAgregar() {
        ...
        JLabel dlgTitle = new JLabel("Nuevo Ingreso");
        ...
        JComboBox<String> cbCat = new JComboBox<>(new String[]{
                "Ventas", "Servicios", "Consultoría", "Mantenimiento", "Licencias", "Otros"});
        ...
        JButton btnSave   = createButton("Guardar",  COLOR_GREEN);
        ...
        btnSave.addActionListener(e -> {
            try {
                ...
                empresa.agregarTransaccion(new Ingreso(nextId, desc, monto, fecha, cat));
                saveCSV();
                build();
                dialog.dispose();
                JOptionPane.showMessageDialog(IngresosPanel.this,
                        "Ingreso registrado correctamente.", "Éxito", JOptionPane.INFORMATION_MESSAGE);

            } catch (NumberFormatException ex) {
                showErr(dialog, "El monto debe ser un número entero válido (ej: 1500000).");
    ...
    private List<Transaccion> getIngresos() {
    ...
    static class DeleteButtonEditor extends DefaultCellEditor {
        ...
        private final IngresosPanel panel;
        private int currentRow;

        public DeleteButtonEditor(JCheckBox checkBox, IngresosPanel panel) {
            super(checkBox);
            this.panel = panel;
            ...
            button.addActionListener(e -> {
                fireEditingStopped();
                panel.eliminarIngreso(currentRow);
            });
        }
